...
Все поля из объекта данных уведомления, включая вложенные свойства:

- Прямые поля: `{name}`, `{level}`, `{type}`
- Вложенные поля через точку: `{data.name}`, `{data.server_name}`
- Поля ошибок: `{error}` (автоматически ищет в `data.err.error`)
- Элементы массивов по индексу: `{images.0}`, `{trace.1}`

Пример шаблона:
```json
{
  "ServerUnreachable": {
    "CRITICAL": "🔴 Сервер {name} недоступен: {error}",
